...
import {
  ...
  Globe2, Instagram, Layers3, Linkedin, Mail, MapPin, Menu,
...
const faqs = [
  ...
  ['How do I contact Rexora Technologies?', 'Email rexoratechnologies@gmail.com or call +1 (240) 251-9611. You can also use the inquiry form below and the Rexora team will follow up.'],
];
...
function Logo({ className = '' }: { className?: string }) {
  return <span className={`logo-frame ${className}`}><img src={rexoraLogo} alt="Rexora Technologies" className="logo-img" /></span>;
}
...
function App() {
  ...
  useEffect(() => {
    document.title = 'Rexora Technologies | Your Next Career Move Starts Here';
    const description = 'Rexora Technologies connects ambitious professionals and businesses with technology expertise, career opportunities, and specialized services.';
  ...
  return <div className="min-h-[100dvh] overflow-x-hidden bg-[#f7faff] text-[#07142f]">
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-[0_8px_30px_rgba(7,20,47,.06)] backdrop-blur-md transition-all duration-300">
      <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 md:px-8 xl:px-12">
        <a href="#home" onClick={closeMenu} className="focus-ring shrink-0 rounded" aria-label="Rexora Technologies home" data-testid="link-logo">
         <Logo className="h-16 w-24" />
        </a>
        <nav className="hidden items-center gap-6 lg:flex" aria-label="Main navigation">
          {['about', 'services', 'solutions', 'industries', 'careers', 'insights'].map((item) => <a key={item} href={`#${item}`} className="focus-ring rounded px-1 py-2 text-[11px] font-bold uppercase tracking-[.12em] text-slate-600 transition-colors hover:text-blue-700" data-testid={`link-nav-${item}`}>{item}</a>)}
        </nav>
        ...
        <button type="button" onClick={() => setMenuOpen(!menuOpen)} className="focus-ring rounded p-2 text-[#07142f] lg:hidden" aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} data-testid="button-mobile-menu">{menuOpen ? <X size={23} /> : <Menu size={23} />}</button>
      </div>
    ...
    <main>
      <section id="home" className="relative isolate min-h-[720px] overflow-hidden bg-[#07142f] pt-32 text-white md:min-h-[780px] md:pt-40">
        ...
        <div className="mx-auto grid max-w-[1440px] items-center gap-16 px-5 pb-24 md:px-8 lg:grid-cols-[1.02fr_.98fr] lg:gap-8 xl:px-12">
          <div className="max-w-2xl">
            ...
            <h1 className="hero-enter-delay max-w-3xl text-[3.25rem] font-extrabold leading-[.98] tracking-[-.065em] sm:text-6xl md:text-7xl xl:text-[5.8rem]">Your Next <span className="text-[#4ba4ff]">Career Move</span> Starts Here</h1>
            <p className="hero-enter-delay-2 mt-5 max-w-xl text-lg font-semibold leading-7 tracking-[-.02em] text-blue-100 md:text-xl" data-testid="text-brand-tagline">Right Talent. Right Technology. Right Opportunity.</p>
            <p className="hero-enter-delay-2 mt-4 max-w-xl text-base leading-8 text-blue-100/75 md:text-lg">Rexora Technologies delivers the software, cloud, data, AI, and specialist expertise that help businesses build, scale, and transform.</p>
            <div className="hero-enter-delay-2 mt-8 flex flex-col gap-3 sm:flex-row">
     ...
     <footer className="bg-[#07142f] px-5 pb-8 pt-16 text-white md:px-8 md:pt-20 xl:px-12"><div className="mx-auto max-w-[1440px]"><div className="grid gap-12 lg:grid-cols-[1.25fr_1fr_1fr_1fr]"><div><a href="#home" className="focus-ring inline-block rounded" data-testid="link-footer-logo"><Logo className="footer-logo h-[84px] w-[126px]" /></a><p className="mt-6 max-w-xs text-sm leading-7 text-blue-100/60">Technology and professional services for businesses ready to build, scale, and transform.</p><div className="mt-7 space-y-2 text-sm text-blue-100/70"><a href="mailto:rexoratechnologies@gmail.com" className="block transition hover:text-white">rexoratechnologies@gmail.com</a><a href="tel:+12402519611" className="block transition hover:text-white">+1 (240) 251-9611</a></div><div className="mt-7 flex gap-3"><a href="https://www.linkedin.com/company/rexora-technologies/" target="_blank" rel="noreferrer" className="focus-ring rounded-full border border-white/15 p-2.5 text-blue-200 transition hover:border-blue-400 hover:text-white" aria-label="LinkedIn" data-testid="link-social-linkedin"><Linkedin size={16} /></a><a href="https://www.instagram.com/rexora.technologies_/" target="_blank" rel="noreferrer" className="focus-ring rounded-full border border-white/15 p-2.5 text-blue-200 transition hover:border-blue-400 hover:text-white" aria-label="Instagram" data-testid="link-social-instagram"><Instagram size={16} /></a><a href="#contact" className="focus-ring rounded-full border border-white/15 p-2.5 text-blue-200 transition hover:border-blue-400 hover:text-white" aria-label="X" data-testid="link-social-x"><Twitter size={16} /></a><a href="#contact" className="focus-ring rounded-full border border-white/15 p-2.5 text-blue-200 transition hover:border-blue-400 hover:text-white" aria-label="Facebook" data-testid="link-social-facebook"><Facebook size={16} /></a></div></div><FooterColumn title="Company" links={['About', 'Careers', 'Insights', 'Contact']} /><FooterColumn title="Services" links={['Software Development', 'Cloud & DevOps', 'Data & AI', 'Consulting', 'Staff Augmentation']} /><FooterColumn title="Resources" links={['Case Studies', 'Blog', 'FAQ', 'Privacy Policy']} /></div><div className="mt-16 flex flex-col justify-between gap-4 border-t border-white/10 pt-6 text-[11px] text-blue-100/45 sm:flex-row"><span>© 2026 Rexora Technologies. All rights reserved.</span><span>Technology • Innovation • Expertise • Results</span></div></div></footer>
  </div>;
}
